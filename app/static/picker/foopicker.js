...
var FooPicker = (function () {
    ...
    _self.showPicker = function () {
      ...
      if (pickerField) {
        ...
        if (pickerDiv) {
          ...
          pickerDiv.style.left = (left - 70) + 'px';
  ...
  var Calendar = {
    ...
    // Build years select
    //acc, 2020, en el for los años atras y años adelante, en el selected, se pone el anio
    //que quermos que este seleccionado
    buildYears: function () {
      //var elem = '<select class="foopicker__date--year">', currentYear = this.getCurrentYear();
      var elem = '<select class="foopicker__date--year">', currentYear = this.year();


      var error_acc = 0;
      var year = this.year();
      try {
        var aux_acc = document.getElementById('st_anio_inicio').value;
        //alert(aux);
      }
      catch (e) {
        error_acc = 1;
      ...
      if (error_acc == 0) {
        year = parseInt(aux_acc) + 2;
      }


      for (var i = year - 2; i <= currentYear + 1; i++) {
        elem += '<option value="' + i + '"';
        if (i === currentYear) {
          elem += ' selected';
        }
  ...
  function addListeners(picker) {
    ...
    //var bt= document.getElementById("btnA");
    if (el) {
      addEvent(el, 'click', picker.showPicker, false);
      //addEvent(bt, 'click', picker.showPicker, false);
