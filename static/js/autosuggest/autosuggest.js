...
function autosuggest(id, url, onSelect) {
	...
	this.response_time = 100;	// time, in milliseconds, between the last char typed and the actual query
	this.entry_limit = 10;	// number of entries autocomplete will show at a time

	//acc, para que busque las letras en medio de las palabras
...
autosuggest.prototype = {
	...
	suggest: function (ot, t) {
		...
		if (!this.field.value.length) {
			...
			/*re1= new RegExp(/buf.*mes.*do/, "i");
			re2= re1;
			var re3= new RegExp(/mes.*buf.*do/, "i");*/
			var palabra = t.trim();
			var listaP = Array();
			listaP = palabra.split(" ");

			var rp1 = new RegExp(t, "i");
			var rp2 = new RegExp(t, "i");
			var rp3 = new RegExp(t, "i");
			var rp4 = new RegExp(t, "i");
			var rp5 = new RegExp(t, "i");
			var rp6 = new RegExp(t, "i");
			...
			if (listaP.length == 1) {
				...
				if (listaP.length == 2) {
					rp1 = new RegExp(listaP[0] + '.*' + listaP[1], "i");
					rp2 = new RegExp(listaP[1] + '.*' + listaP[0], "i");
				}
				else {
					if (listaP.length == 3) {
						rp1 = new RegExp(listaP[0] + '.*' + listaP[1] + '.*' + listaP[2], "i");
						rp2 = new RegExp(listaP[0] + '.*' + listaP[2] + '.*' + listaP[1], "i");
						rp3 = new RegExp(listaP[1] + '.*' + listaP[0] + '.*' + listaP[2], "i");
						rp4 = new RegExp(listaP[1] + '.*' + listaP[2] + '.*' + listaP[0], "i");
						rp5 = new RegExp(listaP[2] + '.*' + listaP[0] + '.*' + listaP[1], "i");
						rp6 = new RegExp(listaP[2] + '.*' + listaP[1] + '.*' + listaP[0], "i");
					}
					else {
						rp1 = new RegExp(t, "i");
					}
			...
			for (var i = 0; i < al; i++) {
				...
				//acc, combinacion
				if (listaP.length != 2 && listaP.length != 3) {
					if (re1.test(key)) {
						++this.total;
					...
					if (listaP.length == 2) {
						if (rp1.test(key) || rp2.test(key)) {
							++this.total;

							matchArray[matchArray.length] = key;
							matchVArray[matchVArray.length] = value;
						}
						else if (after && (rp1.test(key) || rp2.test(key))) {
							++this.total;
					...
					if (listaP.length == 3) {
						if (rp1.test(key) || rp2.test(key) || rp3.test(key) || rp4.test(key) || rp5.test(key) || rp6.test(key)) {
							++this.total;

							matchArray[matchArray.length] = key;
							matchVArray[matchVArray.length] = value;
						}
						else if (after && (rp1.test(key) || rp2.test(key) || rp3.test(key) || rp4.test(key) || rp5.test(key) || rp6.test(key))) {
							++this.total;
