...
def myFirstPage(canvas, doc):
    ...
    datosReporte['titulo'] = 'Movimientos Caja ' + DATO_CAJA
...
def rptMovimientosCaja(buffer_pdf, usuario, ciudad_id, sucursal_id, caja_id, fecha_ini, fecha_fin, anulados):
    ...
    datos_reporte = reporte_controller.datos_movimientos_caja(usuario, ciudad_id, sucursal_id, caja_id, fecha_ini, fecha_fin, anulados)
    # print(datos_reporte)
        ...
        if caja_id != dato['caja1_id']:
            ...
            if bande > 1:
                ...
                if len(data) > 0:
                    ...
                    tabla_datos = Table(data, colWidths=[35*mm, 35*mm, 95*mm, 25*mm], repeatRows=1)
                    tabla_datos.setStyle(TableStyle([('BACKGROUND', (0, 0), (3, 0), colors.Color(red=(204/255), green=(204/255), blue=(204/255))),
                                                     #('ALIGN', (2, 0), (2, -1), 'RIGHT'),
                                                     ('ALIGN', (3, 0), (3, filas+1), 'RIGHT'),
                                                     ('ALIGN', (2, filas+1), (2, filas+1), 'RIGHT'),
            ...
            titulo_caja = dato['punto1'] + ' - ' + dato['caja1']
            caja_id = dato['caja1_id']

            # creamos tabla para esta caja
            data = []
            data.append(['Fecha', 'Caja', 'Concepto', 'Monto'])
            filas = 0
            total = 0

        # seguimos llenando de datos la tabla hasta cambiar de caja, sucursal o ciudad
        if dato['punto2'] == '' and dato['caja2'] == '':
            punto_poner = Paragraph('SIN RECEPCION', style_punto)
        else:
            punto_poner = Paragraph(dato['punto2']+'-'+dato['caja2'])

        concepto = Paragraph(dato['concepto'])

        datos_tabla = [dato['fecha'], punto_poner, concepto, str(dato['monto']) + ' ' + dato['tipo_moneda']]
        data.append(datos_tabla)
        filas += 1
        total += dato['monto']
