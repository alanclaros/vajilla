...
def myFirstPage(canvas, doc):
    ...
    # subtotal
    canvas.drawString(posX2*mm, posY*mm, ' ')
    canvas.drawRightString(posX2*mm, posY*mm, "F. Evento : ")
    #canvas.drawString(posX3*mm, posY*mm, ' ')
    canvas.drawString(posX3*mm, posY*mm, get_date_show(venta.fecha_evento))
    ...
    canvas.drawString(posX2*mm, posY*mm, ' ')
    canvas.drawRightString(posX2*mm, posY*mm, "F. Entrega : ")
    canvas.drawString(posX3*mm, posY*mm, ' ')
    canvas.drawString(posX3*mm, posY*mm, get_date_show(fecha=venta.fecha_entrega, formato='dd-MMM-yyyy HH:ii'))
    ...
    # costo transporte
    canvas.drawString(posX2*mm, posY*mm, ' ')
    canvas.drawRightString(posX2*mm, posY*mm, "F. Devol. : ")
    canvas.drawString(posX3*mm, posY*mm, ' ')
    canvas.drawString(posX3*mm, posY*mm, get_date_show(fecha=venta.fecha_devolucion, formato='dd-MMM-yyyy HH:ii'))
...
def rptVentasSinCostos(buffer_pdf, usuario, venta_id):
    ...
    datos_tabla = []
    data = []
    data.append(['Producto', 'Salida', 'Vuelta', 'Detalle'])
    ...
    for va_aumento in ventas_aumentos:
        ...
        for va_detalle in ventas_aumentos_detalles:
            ...
            if existe > -1:
                ...
                dato['detalle'] = va_detalle.detalle
                productos_detalles.append(dato)

    # cargamos los registros
    for detalle in productos_detalles:
        producto = Paragraph(detalle['producto'], style_tabla_datos)
        para_detalle = Paragraph(detalle['detalle'], style_tabla_datos)
        datos_tabla = []

        datos_tabla = [producto, str(int(detalle['cantidad_salida'])), str(int(detalle['cantidad_vuelta'])), para_detalle]
        data.append(datos_tabla)
        filas += 1
        total += detalle['total_salida']
        total_vuelta += detalle['total_vuelta_rotura']

    tabla_datos = Table(data, colWidths=[110*mm, 12*mm, 12*mm, 67*mm], repeatRows=1)
    num_cols = 4-1
    align_right_from = 1

    tabla_datos.setStyle(TableStyle([('BACKGROUND', (0, 0), (num_cols, 0), colors.Color(red=(204/255), green=(204/255), blue=(204/255))),
                                     # ('ALIGN', (2, 0), (2, -1), 'RIGHT'),
                                     ('ALIGN', (align_right_from, 0), (2, filas), 'RIGHT'),
